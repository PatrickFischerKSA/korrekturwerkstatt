...
function headers(line){const columns=line.items.map(item=>({field:KW.fieldFor(item.s),x:item.x})).filter(c=>c.field&&['number','quote','correction','explanation','weight','type'].includes(c.field));return ['quote','explanation','weight'].every(f=>columns.some(c=>c.field===f))?columns.sort((a,b)=>a.x-b.x):null;}
