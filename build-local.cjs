...
for(const name of ['jszip.min.js','engine.js','pdf-runtime.js','pdf-import.js','app.js'])html=html.replace(new RegExp('<script src="'+name.replace(/\./g,'\\.')+'(?:\\?[^"]*)?"></script>'),()=>'<script>'+fs.readFileSync('dist/'+name,'utf8').replace(/<\/script/gi,'<\\/script')+'</script>');
